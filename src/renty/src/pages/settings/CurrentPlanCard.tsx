  ...
  const handleChangePlan = async (planName: string) => {
    try {
      ...
      const { error } = await authClient.subscription.upgrade({
        plan: targetPlan,
        successUrl: `${origin}/settings?tab=subscription`,
        cancelUrl: `${origin}/settings?tab=subscription`,
      });
  ...
  return (
    <div className="rounded-lg border p-4 bg-card">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        ...
        <Button variant="outline" onClick={() => handleChangePlan(otherPlan?.name || "pro")} disabled={isChangingPlan}>
          {isChangingPlan ? (
            <>
            ...
            currentPlan.name === "basic" ? t("subscription.upgrade-plan") : t("subscription.downgrade-plan")
          )}
        </Button>
      </div>
