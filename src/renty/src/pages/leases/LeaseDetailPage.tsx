...
import type { document as DocumentType, lease, property, tenant, tenantAuth } from "@/lib/types";
...
import LeaseRentReceiptSettings from "./LeaseRentReceiptSettings";
...
export function LeaseDetailPage() {
  ...
  const rawLease = useQuery(api.leases.findForUser, id && userId ? { id, userId } : "skip");
  const lease = useMemo(
    () => rawLease ? reviveDates(rawLease) as unknown as LeaseWithDetails : null,
    [rawLease],
  );
  const rawDocuments = useQuery(
    api.documents.listForProperty,
    lease?.propertyId ? { propertyId: lease.propertyId } : "skip",
  );
  ...
  if (!lease) {
    return (
      <div className="container mx-auto px-4 py-6 sm:px-6 space-y-4 max-w-7xl">
        <Link href="/leases" className="text-sm text-muted-foreground hover:text-foreground flex items-center gap-1">
    ...
    <div className="container mx-auto px-4 py-6 sm:px-6 space-y-6 max-w-7xl">
      ...
      {isExpired && (
        <div className="flex items-center gap-3 rounded-lg border border-destructive/30 bg-destructive/10 px-4 py-3 text-destructive">
          ...
          <div className="flex flex-wrap items-baseline gap-1.5 text-sm">
            ...
            <span className="opacity-80">{t("since")} {format(new Date(lease.endDate!), "dd MMMM yyyy", { locale: fr })}</span>
          </div>
        </div>
      )}
        ...
        <div className="flex items-center gap-3 rounded-lg border border-warning/30 bg-warning-muted px-4 py-3 text-warning-foreground">
          ...
          <div className="flex flex-wrap items-baseline gap-1.5 text-sm">
            ...
            <span className="opacity-80">
              ...
              {daysUntilEnd !== null && <span className="ml-1 font-medium">({daysUntilEnd} jours)</span>}
            </span>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
            ...
            <CardContent className="space-y-5">
              <div className="rounded-xl bg-primary/5 border border-primary/15 px-5 py-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                  ...
                  <p className="text-3xl font-bold text-primary tracking-tight">{formatCurrency(monthlyTotal, lease.currency)}</p>
                ...
                <div className="flex items-center gap-3 rounded-lg border bg-muted/20 px-4 py-3">
                  ...
                  <div>
                    ...
                    <p className="font-semibold text-sm">
                      {lease.charges && lease.charges > 0 ? formatCurrency(lease.charges, lease.currency) : <span className="text-muted-foreground font-normal text-sm">—</span>}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-3 rounded-lg border bg-muted/20 px-4 py-3">
                  ...
                  <div>
                    ...
                    <p className="font-semibold text-sm">
                      {lease.depositAmount ? formatCurrency(lease.depositAmount, lease.currency) : <span className="text-muted-foreground font-normal text-sm">—</span>}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-2 pt-1">
                <Badge variant="secondary" className="font-normal text-xs">{t(`frequency.${lease.paymentFrequency}`)}</Badge>
                <Badge variant="secondary" className="font-normal text-xs">{lease.currency}</Badge>
                {lease.isFurnished && <Badge variant="secondary" className="font-normal text-xs">{t("furnished")}</Badge>}
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="space-y-5">
              <div className="grid grid-cols-1 sm:grid-cols-3 items-center gap-3">
                <div className="rounded-lg border bg-muted/20 px-4 py-3">
                  ...
                  <p className="font-semibold text-sm">{format(new Date(lease.startDate), "dd MMM yyyy", { locale: fr })}</p>
                </div>
                <div className="flex items-center justify-center">
                  {durationMonths !== null ? (
                    <div className="text-center w-full">
                      <p className="text-xs font-medium text-muted-foreground mb-1.5">{durationMonths} {t("months")}</p>
                ...
                <div className="rounded-lg border bg-muted/20 px-4 py-3">
                  ...
                  <p className="font-semibold text-sm">
                    {lease.endDate ? format(new Date(lease.endDate), "dd MMM yyyy", { locale: fr }) : <span className="text-muted-foreground">∞</span>}
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-6 gap-y-4">
                <div>
                  ...
                  <Badge className={cn(getLeaseTypeColor(lease.leaseType), "font-normal text-xs")}>{t(`type.${lease.leaseType.toLowerCase()}`)}</Badge>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="text-sm font-medium">{lease.isFurnished ? t("furnished-yes") : t("furnished-no")}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground mb-1.5">{t("created-on")}</p>
                  <p className="text-sm font-medium">{format(new Date(lease.createdAt), "dd MMM yyyy", { locale: fr })}</p>
                </div>
              </div>
              ...
              {lease.notes && (
                <>
                  ...
                  <div>
                    ...
                    <p className="text-sm bg-muted/30 rounded-lg px-4 py-3 leading-relaxed">{lease.notes}</p>
                  </div>
                </>
              )}
            </CardContent>
              ...
              <div className="flex items-center justify-between gap-4">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground truncate">{lease.property.address}, {lease.property.postalCode} {lease.property.city}</p>
                  </div>
                </div>
          ...
          <LeaseDocumentsSection lease={lease} documents={documents} />
        </div>
            ...
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <CardTitle className="flex items-center gap-2 text-base font-semibold">
                  ...
                  {t("tenants")}
                  {lease.tenants.length > 0 && <span className="text-sm font-normal text-muted-foreground">({lease.tenants.length})</span>}
                </CardTitle>
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between gap-2 mb-0.5">
                            <p className="font-semibold text-sm truncate">{tenant.firstName} {tenant.lastName}</p>
                            {tenant.auth?.isActivated ? (
                              <Badge className="text-xs bg-success-muted text-success-foreground hover:bg-success-muted flex-shrink-0">{t("activated")}</Badge>
                            ) : (
                              <Badge variant="outline" className="text-xs flex-shrink-0">{t("pending")}</Badge>
                            )}
                          </div>
                          <p className="text-xs text-muted-foreground truncate">{tenant.email}</p>
                          {tenant.phoneNumber && <p className="text-xs text-muted-foreground">{tenant.phoneNumber}</p>}
                          {tenant.notes && <p className="text-xs text-muted-foreground mt-1.5 italic leading-relaxed">{tenant.notes}</p>}
                        </div>
                      </div>
                    </div>
                  ))}
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-2 gap-4">
                <StatChip label={t("duration")} value={durationMonths !== null ? `${durationMonths} ${t("months")}` : "∞"} />
                <StatChip label={t("annual-revenue")} value={formatCurrency(annualRevenue, lease.currency)} />
                <StatChip label={t("created-on")} value={format(new Date(lease.createdAt), "dd/MM/yyyy")} />
                {daysUntilEnd !== null && daysUntilEnd > 0 && (
                  <StatChip label={t("days-remaining")} value={`${daysUntilEnd}j`} sub={format(new Date(lease.endDate!), "dd MMM yyyy", { locale: fr })} />
                )}
              </div>
            </CardContent>
          </Card>
